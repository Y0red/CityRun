...
public abstract class EngineManager : MonoBehaviour
{
    ...
    private void AssignAllAudio()
    {
        ...
        skidSound = audioSources[3];
        windSound = audioSources[4];
        brakeSound = audioSources[5];
        crashSound = audioSources[6];
        ...
        crashSound = CreateAudioSource.NewAudioSource(crashSound, "Crash Sound AudioSource", 5, 0, audio_SO.crashClips[1], true, true, false);
    }
    #endregion
    ...
    {
        float newInput;
        if (inputManager.LocalInput == 1) newInput = 1;

        else if (inputManager.LocalInput == -1) newInput = 1;
        else newInput = 0;
        ...
        if (engineSoundOn)
        {
            if (!engine_SO.reversing) 
                engineSoundOn.volume = Mathf.Lerp(engineSoundOn.volume, Mathf.Clamp((newInput), audio_SO.minEngineSoundVolume, audio_SO.maxEngineSoundVolume), Time.deltaTime * 50f);
            else 
                engineSoundOn.volume = Mathf.Lerp(engineSoundOn.volume, Mathf.Clamp((newInput), audio_SO.minEngineSoundVolume, audio_SO.maxEngineSoundVolume), Time.deltaTime * 50f);
        ...
        {
           skidSound.volume = Mathf.Lerp(1f, audio_SO.maxBrakeSoundVolume, Mathf.Clamp01(engine_SO.brakForce) * Mathf.Lerp(0f, 1f, allWheels[0].rpm / 50f));
           
        }
        else { skidSound.volume = 0f; }
