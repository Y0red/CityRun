...
{    
    ...
    private void Start()
    {
        //if(isFirstTime) lastPos = this.transform.position;
        objPool = GetComponent<ObjPool>();
        
        priviousDirection = Directions.Forward;

        // GameEvents.Instance.OnGeneratePlatform += Starting;
        // GameEvents.Instance.onRoadWayTriggerEnter += OnIN;
        // GameEvents.Instance.onRoadWayTriggerExit += OnOUT;
        // GameEvents.Instance.OnGeneratePlatform += Starting;
    ...
    {
        GeneratePlatforms();
        //GameEvents.Instance.OnPlatformGenerated(first.playerStartingPos);
    }
    ...
    {
       // if (!isFirstTime && last != null) lastPos = last.gameObject.transform.position;
